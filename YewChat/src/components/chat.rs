        ...
        html! {
            <div class="flex w-screen">
                <div class="flex-none w-56 h-screen bg-blue-100">
                    <div class="text-xl p-3 text-blue-900">{"Users"}</div>
                    {
                                ...
                                <div class="flex m-3 bg-blue-50 rounded-lg p-2 shadow-lg">
                                    ...
                                    <div class="flex-grow p-3">
                                        <div class="flex text-xs justify-between">
                                            <div class="text-blue-800">{u.name.clone()}</div>
                                        </div>
                ...
                <div class="grow h-screen flex flex-col bg-gray-50">
                    <div class="w-full h-14 border-b-2 border-blue-300 bg-white"><div class="text-xl p-3 text-gray-700">{"💬 Let's Chat!"}</div></div>
                    <div class="w-full grow overflow-auto">
                        {
                            self.messages.iter().map(|m| {
                                let user = self.users.iter().find(|u| u.name == m.from).unwrap();
                                html!{
                                    <div class="flex items-end w-3/6 m-8 rounded-lg shadow-lg" style="background-color: #b6d0e2;">
                                        ...
                                        <div class="p-3">
                                            ...
                                            <div class="text-xs text-gray-600">
                                                if m.message.ends_with(".gif") {
                                                    <img class="mt-3 rounded-md" src={m.message.clone()}/>
                                                } else {
                                                    {m.message.clone()}
                                                }
                                            </div>
